  ...
  <div>
     <q-tabs
      ...
      >
          ...
          <q-tab :name="2" label="Formato 1009"  @click="armarColums(2)"/>
    ...
    <q-tab-panels v-model="tab" animated class="bg-white text-primary q-pa-none" >
...
export default {
  ...
  methods: {
    async getLoad (params) {
      await this.Get(`contabilidad/exogena/search/${1009}/${params}/${this.inAnio}`).then((resp) => {
        if (this.tab === 4) {
  ...
  watch: {
    'onClick' () {
      this.LConfim('¿Desea exportar en formato excel?', async () => await this.PostDownload('contabilidad/exogena/exportar_excel_exogena', { datos: this.datos, encabezado: `${this.tab === 1 ? 'Movimiento Detallado 1009' : this.tab === 2 ? 'Formato 1009' : this.tab === 3 ? 'Cuantías Menores 1009' : 'Parametros 1009'}` }).then((resp) =>
        fileDownload(resp, `${this.tab === 1 ? 'MovimientoDetallado1009.xlsx' : this.tab === 2 ? 'Formato1009.xlsx' : this.tab === 3 ? 'CuantíasMenores1009.xlsx' : 'Parametros1009.xlsx'}`))
      )
    }
  },
  mounted () {
