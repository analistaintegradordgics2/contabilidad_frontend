  ...
  <div>
     <q-tabs
      ...
      >
          ...
          <q-tab :name="2" label="Formato 5248"  @click="armarColums(2)"/>
      </q-tabs>
...
export default {
  ...
  methods: {
    async getLoad (params) {
      await this.Get(`contabilidad/exogena/search/${5248}/${params}/${this.inAnio}`).then((resp) => {
        this.datos = resp ?? []
      })
    },
  ...
  watch: {
    'onClick' () {
      this.LConfim('¿Desea exportar en formato excel?', async () => await this.PostDownload('contabilidad/exogena/exportar_excel_exogena', { datos: this.datos, encabezado: `${this.tab === 1 ? 'Movimiento Detallado 5248' : this.tab === 2 ? 'Formato 5248' : 'Parametros 5248'}` }).then((resp) =>
        fileDownload(resp, `${this.tab === 1 ? 'MovimientoDetallado5248.xlsx' : this.tab === 2 ? 'Formato5248.xlsx' : 'Parametros5248.xlsx'}`))
      )
    }
  },
  mounted () {
