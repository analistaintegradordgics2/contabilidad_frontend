  ...
  <div>
     <q-tabs
      ...
      >
          ...
          <q-tab :name="2" label="Formato 1001"  @click="armarColums(2)"/>
    ...
    <q-tab-panels v-model="tab" animated class="bg-white text-primary">
      ...
      <q-tab-panel :name="4" class="q-pa-none">
          <tabParametros :columns="columnsData" :datos="datos" :mayor="mayor" :valCuantia="valCuantia" :formato="formato" @update-detail="updateDetail" @update-cuantia="updateCuantia" @delete-params="deleteParams" @add-params="onAddParams"/>
       </q-tab-panel>
    </q-tab-panels>
    </div>
</template>
...
export default {
  ...
  methods: {
    async getLoad (params) {
      await this.Get(`contabilidad/exogena/search/${1001}/${params}/${this.inAnio}`).then((resp) => {
        if (this.tab === 4) {
  ...
  watch: {
    async 'inAnio' () {
      await this.armarColums(1, this.formato)
    },
    'onClick' () {
      this.LConfim('¿Desea exportar en formato excel?', async () => await this.PostDownload('contabilidad/exogena/exportar_excel_exogena', { datos: this.datos, encabezado: `${this.tab === 1 ? 'Movimiento Detallado 1001' : this.tab === 2 ? 'Formato 1001' : this.tab === 3 ? 'Cuantías Menores 1001' : 'Parametros 1001'}` }).then((resp) =>
        fileDownload(resp, `${this.tab === 1 ? 'MovimientoDetallado1001.xlsx' : this.tab === 2 ? 'Formato1001.xlsx' : this.tab === 3 ? 'CuantíasMenores1001.xlsx' : 'Parametros1001.xlsx'}`))
      )
    }
  },
  mounted () {
