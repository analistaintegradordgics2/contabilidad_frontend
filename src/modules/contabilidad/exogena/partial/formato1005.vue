  ...
  <div>
     <q-tabs
      ...
      >
          ...
          <q-tab :name="2" label="Formato 1005"  @click="armarColums(2)"/>
...
export default {
  ...
  methods: {
    async getLoad (params) {
      await this.Get(`contabilidad/exogena/search/${1005}/${params}/${this.inAnio}`).then((resp) => {
        this.datos = resp ?? []
      })
    },
  ...
  watch: {
    'onClick' () {
      this.LConfim('¿Desea exportar en formato excel?', async () => await this.PostDownload('contabilidad/exogena/exportar_excel_exogena', { datos: this.datos, encabezado: `${this.tab === 1 ? 'Movimiento Detallado 1005' : this.tab === 2 ? 'Formato 1005' : 'Parametros 1005'}` }).then((resp) =>
        fileDownload(resp, `${this.tab === 1 ? 'MovimientoDetallado1005.xlsx' : this.tab === 2 ? 'Formato1005.xlsx' : 'Parametros1005.xlsx'}`))
      )
    }
  },
  mounted () {
