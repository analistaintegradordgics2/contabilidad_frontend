  ...
  <div>
     <q-tabs
      ...
      >
          ...
          <q-tab :name="2" label="Formato 5252"  @click="armarColums(2)"/>
...
export default {
  ...
  methods: {
    async getLoad (params) {
      await this.Get(`contabilidad/exogena/search/${5252}/${params}/${this.inAnio}`).then((resp) => {
        if (this.tab === 3) {
  ...
  watch: {
    'onClick' () {
      this.LConfim('¿Desea exportar en formato excel?', async () => await this.PostDownload('contabilidad/exogena/exportar_excel_exogena', { datos: this.datos, encabezado: `${this.tab === 1 ? 'Movimiento Detallado 5252' : this.tab === 2 ? 'Formato 5252' : 'Parametros 5252'}` }).then((resp) =>
        fileDownload(resp, `${this.tab === 1 ? 'MovimientoDetallado5252.xlsx' : this.tab === 2 ? 'Formato5252.xlsx' : 'Parametros5252.xlsx'}`))
      )
    }
  },
  mounted () {
